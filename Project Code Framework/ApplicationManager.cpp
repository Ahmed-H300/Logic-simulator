...
void ApplicationManager::UpdateInterface()
{
	for (int i = 0; i < CompCount; i++)
	{
		...
		if (p != NULL)
		{
			CompList[i]->drawlable(OutputInterface, CompList[i]->get_label(), (CompList[i]->get_GraphicsInfo().x2+ CompList[i]->get_GraphicsInfo().x1)/2, CompList[i]->get_GraphicsInfo().y2);
		}
		else
		{
...
void ApplicationManager::dsn_mode()
{
	for (int i = 0; i < CompCount; i++)
	{
		LED* led = dynamic_cast<LED*>(CompList[i]);
